...
class SuspensionDecoration(context: Context, private var mDatas: List<ISuspensionInterface>?) :
    ...
    private val mBounds //用于存放测量文字Rect
            : Rect
    private val mInflater: LayoutInflater
    private var mTitleHeight //title的高
            : Int
    ...
    private fun drawTitleArea(
        ...
        int baseline = (getMeasuredHeight() - fontMetrics.bottom + fontMetrics.top) / 2 - fontMetrics.top;*/mPaint.getTextBounds(
            mDatas!![position].suspensionTag, 0, mDatas!![position].suspensionTag!!.length, mBounds
        )
        c.drawText(
    ...
    override fun onDrawOver(
        ...
        val child =
            parent.findViewHolderForLayoutPosition(pos + headerViewCount)!!.itemView //出现一个奇怪的bug，有时候child为空，所以将 child = parent.getChildAt(i)。-》 parent.findViewHolderForLayoutPosition(pos).itemView
        ...
        if (pos + 1 < mDatas!!.size) { //防止数组越界（一般情况不会出现）
            if (null != tag && tag != mDatas!![pos + 1].suspensionTag) { //当前第一个可见的Item的tag，不等于其后一个item的tag，说明悬浮的View要切换了
                Log.d(
                    "zxt",
                    "onDrawOver() called with: c = [" + child.top
