...
//                                mLocationManager?.removeUpdates(listener);
    ...
    fun register(minTime: Long, minDistance: Long, listener: OnLocationChangeListener?): Boolean {
        ...
        mLocationManager!!.requestLocationUpdates(
            provider, minTime, minDistance.toFloat(),
            myLocationListener!!
        )
        return true
    }
    ...
    fun isBetterLocation(newLocation: Location, currentBestLocation: Location?): Boolean {
        ...
        val timeDelta: Long = newLocation.getTime() - currentBestLocation.getTime()
        ...
        val accuracyDelta = (newLocation.getAccuracy() - currentBestLocation.getAccuracy()) as Int
        val isLessAccurate = accuracyDelta > 0
        val isMoreAccurate = accuracyDelta < 0
        val isSignificantlyLessAccurate = accuracyDelta > 200

        // Check if the old and new location are from the same provider
        val isFromSameProvider = isSameProvider(newLocation.getProvider(), currentBestLocation.getProvider())
