...
final class CustomMaterialCalendarGridView extends GridView {

  private final Calendar dayCompute = CustomUtcDates.getUtcCalendar();
  private final boolean nestedScrollable;

  public CustomMaterialCalendarGridView(Context context) {
    this(context, null);
  }

  public CustomMaterialCalendarGridView(Context context, AttributeSet attrs) {
    this(context, attrs, 0);
  }

  public CustomMaterialCalendarGridView(Context context, AttributeSet attrs, int defStyleAttr) {
    super(context, attrs, defStyleAttr);
//    if (MaterialDatePicker.isFullscreen(getContext())) {
      setNextFocusLeftId(R.id.cancel_button);
      setNextFocusRightId(R.id.confirm_button);
//    }
    nestedScrollable = true;//MaterialDatePicker.isNestedScrollable(getContext());
  ...
  @NonNull
  @Override
  public CustomMonthAdapter getAdapter() {
    return (CustomMonthAdapter) super.getAdapter();
  }

  @Override
  public final void setAdapter(ListAdapter adapter) {
    if (!(adapter instanceof CustomMonthAdapter)) {
      throw new IllegalArgumentException(
          String.format(
              "%1$s must have its Adapter set to a %2$s",
                  CustomMaterialCalendarGridView.class.getCanonicalName(),
                  CustomMonthAdapter.class.getCanonicalName()));
    }
  ...
  protected final void onDraw(@NonNull Canvas canvas) {
    super.onDraw(canvas);
    CustomMonthAdapter monthAdapter = getAdapter();
    DateSelector<?> dateSelector = monthAdapter.dateSelector;
    CustomCalendarStyle calendarStyle = monthAdapter.calendarStyle;
    ...
    for (Pair<Long, Long> range : dateSelector.getSelectedRanges()) {
      ...
      boolean isRtl = CustomViewUtils.isLayoutRtl(this);
