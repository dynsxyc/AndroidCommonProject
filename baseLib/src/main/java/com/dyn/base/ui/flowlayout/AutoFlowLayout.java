 ...
 * BLOG https://www.jianshu.com/p/d5ff3cb5131a
 */
public class AutoFlowLayout extends ViewGroup {
     ...
     * 子View的点击事件
     */
    private OnAutoFlowItemClickListener mOnItemClickListener;
    /**
     * 当前view的索引
     */
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        mHorizontalSpace = ta.getDimensionPixelSize(R.styleable.AutoFlowLayout_horizontalSpace, 0);
        mVerticalSpace = ta.getDimensionPixelSize(R.styleable.AutoFlowLayout_verticalSpace, 0);
        ...
        mCutLineWidth = ta.getDimensionPixelSize(R.styleable.AutoFlowLayout_cutLineWidth, 1);
    ...
    private void setFlowMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < cCount; i++) {
            ...
            if (lineWidth + childWidth > sizeWidth) {
                ...
                mCount++;
                Logger.i("autoLayout onMeasure-count>" + mCount + "---i->" + i + "---childCount->" + cCount);
...
//        mCheckedViews.clear();
        ...
        for (int i = 0; i < cCount; i++) {
            ...
            if (childWidth + lp.leftMargin + lp.rightMargin + getPaddingRight() + lineWidth > width) {
                ...
                mCount++;
                Logger.i("autoLayout onLayout-count>" + mCount + "---i->" + i + "---childCount->" + cCount);
                if (mCount >= mMaxLineNumbers) {
                    setHasMoreData(i + 1, cCount);
                    break;
                }
    ...
    public void refreshCheckedView() {
        if (mCheckedViews == null || mCheckedViews.isEmpty()) {
            return;
        }
        Iterator<View> iterator = mCheckedViews.iterator();
        while (iterator.hasNext()) {
            if (!contentHasCheckedView(iterator.next())) {
                iterator.remove();
            }
        }
    }
    ...
    public void removeSelectInPosition(View position) {
     ...
     * 整体view中是否包含选中的View
     */
    private boolean contentHasCheckedView(View view) {
        boolean hasCheked = false;
        for (int i = 0; i < getChildCount(); i++) {
            View childView = getChildAt(i);
            if (childView == view && view.isSelected()) {
                hasCheked = true;
            }
        }
        return hasCheked;
     ...
     * 子View 是否选中状态
     */
    public boolean childViewIsSelected(View view) {
        ...
        child.setOnLongClickListener(new OnLongClickListener() {
            ...
            public boolean onLongClick(View view) {
                if (mOnLongItemClickListener != null) {
                    mOnLongItemClickListener.onLongItemClick(getViewPosition(view), view);
                    return true;
                }
                return false;
            }
        ...
        child.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View view) {
                if (mOnItemClickListener != null) {
                    mOnItemClickListener.onFlowItemClick(getViewPosition(view), view);
                }
            }
        });
    }
    ...
    private void setHasMoreData(int i, int count) {
        mHasMoreData = i<=count;
        Log.i("autoLayout","setHasMore->"+mHasMoreData);
    }
    ...
    public void notifyDataSetChange() {
        if (mAdapter.getCount() != 0) {
            for (int i = 0; i < mAdapter.getCount(); i++) {
                View view = mAdapter.getView(mAdapter.getItem(i));
                addView(view);
            }
            requestLayout();
        }
    ...
    public interface OnAutoFlowItemClickListener {
        void onFlowItemClick(int position, View view);
    }

    public void setOnItemClickListener(OnAutoFlowItemClickListener onItemClickListener) {
