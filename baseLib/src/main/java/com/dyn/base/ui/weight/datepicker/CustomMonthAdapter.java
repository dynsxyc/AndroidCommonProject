 ...
 */
class CustomMonthAdapter extends BaseAdapter {

  /**
   * The maximum number of weeks possible in any month. 6 for {@link java.util.GregorianCalendar}.
   */
  static final int MAXIMUM_WEEKS = CustomUtcDates.getUtcCalendar().getMaximum(Calendar.WEEK_OF_MONTH);

  /** The maximum number of cells needed in the month grid view. */
  private static final int MAXIMUM_GRID_CELLS =
          CustomUtcDates.getUtcCalendar().getMaximum(Calendar.DAY_OF_MONTH)
          + CustomUtcDates.getUtcCalendar().getMaximum(Calendar.DAY_OF_WEEK)
          - 1;
  ...
  final CustomCalendarConstraints calendarConstraints;

  @Nullable final CustomDayViewDecorator dayViewDecorator;

  CustomMonthAdapter(
          CustomMonth month,
      DateSelector<?> dateSelector,
          CustomCalendarConstraints calendarConstraints,
   ...
   *     first week of the month represented by {@link CustomMonth} or {@link
   *     CustomCalendarConstraints#getFirstDayOfWeek()} if set.
  ...
  public TextView getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
    ...
    if (offsetPosition < 0 || offsetPosition >= month.daysInMonth) {
      ...
      if (month.year == CustomMonth.current().year) {
        dayTextView.setContentDescription(CustomDateStrings.getMonthDayOfWeekDay(dayInMillis));
      } else {
        dayTextView.setContentDescription(CustomDateStrings.getYearMonthDayOfWeekDay(dayInMillis));
      }
  ...
  public void updateSelectedStates(CustomMaterialCalendarGridView monthGrid) {
  ...
  private void updateSelectedStateForDate(CustomMaterialCalendarGridView monthGrid, long date) {
    if (CustomMonth.create(date).equals(month)) {
  ...
  private void updateSelectedState(@Nullable TextView dayTextView, long date, int dayNumber) {
    ...
    final CustomCalendarItemStyle style;
    ...
    if (valid) {
      ...
      } else if (CustomUtcDates.getTodayCalendar().getTimeInMillis() == date) {
  ...
  private boolean isSelected(long date) {
    for (long selectedDay : dateSelector.getSelectedDays()) {
      if (CustomUtcDates.canonicalYearMonthDay(date) == CustomUtcDates.canonicalYearMonthDay(selectedDay)) {
        return true;
      }
    }
    return false;
  ...
  private void initializeStyles(Context context) {
    if (calendarStyle == null) {
      calendarStyle = new CustomCalendarStyle(context);
    }
  }
   ...
   *     less than {@link CustomMonthAdapter#firstPositionInMonth()}.
   */
   ...
   * CustomMonthAdapter#getItemId(int)}.
   */
